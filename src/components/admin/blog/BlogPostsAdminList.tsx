...
export function BlogPostsAdminList() {
  ...
  const fetchPosts = useCallback(async () => {
    let rows: BlogPostAdminItem[];
    try {
      rows = await listBlogPosts();
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-[960px]">
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-text">Blog posts</h1>
      ...
      {state === 'loading' && (
        <div className="flex items-center gap-3 py-16">
          ...
          <span className="font-mono text-sm text-text-muted">loading posts…</span>
        </div>
      )}
              ...
              {posts.map((post) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-text-dim">{post.slug}</p>
                  </td>
